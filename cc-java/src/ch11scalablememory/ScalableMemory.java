...
// TBD 3
/**
 * 11.3 40억 개의 0을 포함하는 양의 정수가 들어있는 파일이 있다.
 ...
 *
 * (4E)
 * 12.3 Given an input file with four billion integers,
...
// TBD 4
/**
 * 11.4 1부터 N까지의 숫자가 든 배열이 있다. N은 최대 32,000 이다.
 *      이 배열에는 중복된 숫자가 있으며, N 이 얼마인지는 모른다.
 *      메모리는 4Kbytes 로 제한되어 있다.
 *      배열에 있는 모든 중복 원소를 출력하려면 어떻게 하겠는가?
 */

// WIP 5
/**
 * 11.5 웹에 있는 데이터를 긁어오는 크롤러(crawler)를 설계할 때, 무한 루프(infinite loop)에 빠지는 걸 방지하려면 어떻게 해야 하겠는가?
 */

// TBD 6
/**
 * 11.6 100억개의 URL이 있다. 중복된 문서를 찾으려면 어떻게 해야 하겠는가?
 *      '중복'은 '같은 URL' 이라는 뜻이다.
 */

// TBD 7
/**
 * 11.7 단순화된 검색 엔진을 탑재한 웹 서버를 상상해 보자.
 *      검색 질의(search query)에 답하기 위한 1000개의 기계가 있다.
 ...
 *      가장 최근에 주어진 질의의 처리 결과를 캐싱(caching)하는 방법을 설계하라.
